...
//
// [llamafile] Upstream carried this tool until b10441 and then replaced it
// with scripts/ui-assets.cmake, which generates the same ui.cpp / ui.h from
// tools/ui/ui.{cpp,h}.in inside a CMake build. The cosmocc make build has no
// CMake step, so llamafile keeps the standalone tool: this is upstream's
// b10441 tools/ui/embed.cpp, unchanged. It emits the same interface the
// templates do (llama_ui_find_asset / llama_ui_get_assets / llama_ui_use_gzip
// and LLAMA_UI_HAS_ASSETS), which is what tools/server/server-http.cpp uses.
// Keep its required_check[] table in sync with ui_validate_assets() in
// scripts/ui-assets.cmake and with ui_missing_assets in fetch-ui-assets.sh.
