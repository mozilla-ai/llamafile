...
| `ggml_src_ggml.c.patch` | Makes the MSVC `ggml_time_ms()`/`ggml_time_us()` call `ggml_time_init()` lazily (an idempotent `InitOnceExecuteOnce`). The Windows GPU DLLs built by `cuda*.bat`/`rocm*.bat`/`vulkan.bat` link their own copy of `ggml.c`, so the DLL's `timer_freq` is never set by the executable's `ggml_init()` and any `ggml_time_*()` call made from inside the DLL divides by zero (`0xC0000094`, surfaced by Cosmopolitan as SIGFPE). Latent in every Windows release; first hit when #1051 enabled `-DGGML_CUDA_USE_GRAPHS`, which compiles in upstream's LRU graph eviction (`b94050e89`, b8832), the first caller of `ggml_time_us()` in `ggml-cuda`, and crashed every Windows CUDA run at the warmup decode. Linux/macOS are unaffected (`clock_gettime`, no static divisor). Lets CUDA/HIP graphs stay enabled on Windows as upstream does |
