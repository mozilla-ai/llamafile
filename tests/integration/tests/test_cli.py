...
class TestCLIThinking:
    ...
    def test_nothink_removes_thinking(self, llamafile):
        """Test that --nothink removes thinking content from output."""
        prompt = "What is 2+2? Think step by step then give the answer."

        # With thinking
        result_think = llamafile.run_cli(prompt, nothink=False)

        # Without thinking
        result_nothink = llamafile.run_cli(prompt, nothink=True)
    ...
    def test_nothink_shorter_output(self, llamafile):
