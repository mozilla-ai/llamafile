...
if /i "%~1"=="--help" (
    ...
    echo   -jN              Use N parallel jobs ^(default: NUMBER_OF_PROCESSORS^)
...
set "COMMON_FLAGS=%COMMON_FLAGS% -DGGML_CUDA_USE_GRAPHS"
...
if "%FA_ALL_QUANTS%"=="1" (
    set "FA_COMBINATIONS="
    for %%v in (%FA_TYPES%) do for %%k in (%FA_TYPES%) do call set "FA_COMBINATIONS=%%FA_COMBINATIONS%% %%k-%%v"
) else (
