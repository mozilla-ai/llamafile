...
for arg in "$@"; do
    case "$arg" in
        ...
        --help)
            ...
            echo "  --compress       Force --compress-mode=size (CUDA >=12.8)"
            echo "  --no-compress    Disable --compress-mode=size"
            exit 0
            ;;
        *)
            ARGS+=("$arg")
...
if [ "$MINIMAL_ARCHS" = "1" ]; then
    # Virtual PTX for least-used archs (JIT compiled on first run),
...
# --compress-mode=size: requires CUDA >= 12.8
# Enabled by default for CUDA >= 12.8, overridable with --compress / --no-compress
if [ -z "$COMPRESS" ]; then
    # Auto-detect: enable for CUDA >= 12.8
